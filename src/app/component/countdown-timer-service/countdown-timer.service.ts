...
export class CountdownTimerService {
  ...
  test() {
    interval(100)
      .pipe(take(this.newtotal))
      .map((v) => this.newtotal - v)
      .subscribe(
        (v) => {
          this.timeNowInMinutes =
            (Math.floor(v / 600) % 60) +
            ':' +
            Math.floor((v / 10) % 60) +
            ':' +
  ...
  newTimer(current?) {
    ...
    if (!current) {
      ...
      this.timerList.splice(
        this.timerList.findIndex((t) => t.id === current.id)
      );
